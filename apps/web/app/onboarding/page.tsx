...
export default function OnboardingPage() {
  ...
  async function submitAccount(form: FormData) {
    ...
    try {
      ...
      setError(
        messageFor(
          e,
          'Không tạo được tài khoản. Kiểm tra email và mật khẩu (≥ 8 ký tự), PIN 4–8 số.',
        ),
      );
    } finally {
      setBusy(false);
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-slate-50 via-white to-sky-50">
      <main className="mx-auto flex min-h-screen max-w-2xl flex-col justify-center gap-6 p-6 sm:p-10">
        ...
        {step === 'account' ? (
          <Card>
            <form action={submitAccount} className="flex flex-col gap-4">
              <Field
                id="familyName"
                name="familyName"
                label="Tên gia đình"
                required
                defaultValue="Gia đình"
              />
          ...
          <Card>
            <form action={submitChild} className="flex flex-col gap-4">
              ...
              <Field
                id="interest"
                name="interest"
                label="Bé thích gì? (không bắt buộc)"
                placeholder="ví dụ: vẽ"
              />
              <fieldset className="rounded-lg border border-slate-200 p-3 text-sm">
                ...
                <p className="mb-2 text-slate-500">
                  Xử lý dữ liệu học tập cơ bản là bắt buộc để dùng ứng dụng. Các mục dưới đây tùy
                  chọn, có thể tắt sau.
                </p>
