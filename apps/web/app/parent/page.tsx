...
export default function ParentHome() {
  ...
  async function startForChild(id: string) {
    ...
    try {
      ...
      try {
        if (res.childContext) sessionStorage.setItem('tiny_child_ctx', JSON.stringify(res.childContext));
  ...
  return (
    <main className="mx-auto flex min-h-screen max-w-lg flex-col gap-4 p-6">
      <header className="flex items-center justify-between">
        <h1 className="text-xl font-bold">Nhà của {me?.role === 'FAMILY_OWNER' ? 'bạn' : 'gia đình'}</h1>
        <span className="flex gap-3 text-sm">
          <Link href="/studio" className="text-sky-700 underline" data-testid="studio-link">
            Xưởng nội dung
          </Link>
          <Link href="/parent/admissions" className="text-sky-700 underline" data-testid="admissions-link">
            Quy chế tuyển sinh
          </Link>
        </span>
      ...
      {children.length === 0 ? (
        ...
        <ul className="flex flex-col gap-3">
          <li>
            <label className="flex items-center gap-2 text-sm text-slate-600" data-testid="timed-toggle">
              <input type="checkbox" checked={timed} onChange={(e) => setTimed(e.target.checked)} />
              Luyện có tính giờ (đồng hồ đếm ngược theo thời lượng phiên; hết giờ chỉ nhắc nhẹ, không chấm điểm)
            </label>
          </li>
          {children.map((c) => (
            <li key={c.id}>
